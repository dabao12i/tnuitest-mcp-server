...
// TNUI color themes
export const colorThemes: Record<string, ColorScheme> = {
  default: {
    primary: '#01BEFF',
    success: '#07C160',
    warning: '#FFB700',
...
export const componentStyles: ComponentStyle[] = [
  {
    component: 'tn-button',
    variants: [
      {
...
export const borderRadius: Record<string, { name: string; value: string }> = {
...
export const sizePresets: Record<
  string,
  { name: string; height: string; fontSize: string }
> = {
...
export function detectStyles(description: string): string[] {
...
export function getStyleRecommendation(
  ...
  const styles = componentStyles.find((s) => s.component === component)
  if (!styles) return []

  const detected = detectStyles(description)
  if (detected.length === 0) return styles.variants.slice(0, 3)

  return styles.variants.filter((v) => detected.includes(v.variant))
}

// Generate style props code
export function generateStyleProps(
  component: string,
  description: string
): string {
