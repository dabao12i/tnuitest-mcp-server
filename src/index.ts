...
import { formatSearchResults, searchComponents } from './search-components'
...
function createToolPropertySchema(schema: z.ZodTypeAny): ToolPropertySchema {
...
function createToolSchema(inputSchema: z.ZodSchema<any>) {
  const shape = (inputSchema as z.ZodObject<any>).shape

  return {
    type: 'object',
    properties: Object.fromEntries(
      Object.entries(shape).map(([key, schema]) => {
        const definition = schema as z.ZodTypeAny
...
class TNUIMCPServer {
  ...
  private resources: Map<string, TNUIResource> = new Map()
  ...
  private setupResources() {
    ...
    this.resources.set('tuniao://tnui/components', {
      ...
      description:
        'Complete index of all TNUI (图鸟UI) components organized by category. Use this resource when: 1) User wants to see all available components 2) User needs to browse components by category 3) User asks "有哪些组件", "所有组件列表", "component list"',
    ...
    // Add overview
    const overviewDoc = embeddedDocs.find(
      (d) => d.uri === 'tuniao://tnui/overview'
    )
    if (overviewDoc) {
      this.resources.set('tuniao://tnui/overview', {
        ...overviewDoc,
        description:
          'TNUI library overview and architecture. Use this resource when: 1) User asks about TNUI structure or architecture 2) User needs to understand TNUI core concepts',
      })
    }

    // Add maintenance guide
    const maintenanceDoc = embeddedDocs.find(
      (d) => d.uri === 'tuniao://tnui/maintenance'
    )
    if (maintenanceDoc) {
      this.resources.set('tuniao://tnui/maintenance', {
        ...maintenanceDoc,
        description:
          'TNUI documentation maintenance guidelines. Use this resource when: 1) User needs to update or maintain TNUI documentation 2) User asks about documentation standards',
      })
    }
    ...
    this.resources.set('tuniao://tnui/page-templates', {
      ...
      description:
        'Pre-built uniapp page templates with recommended TNUI components for common page types',
    ...
    this.resources.set('tuniao://tnui/styles', {
      ...
      description:
        'TNUI component style variants, color themes, and design tokens',
      mimeType: 'application/json',
  ...
  private setupTools() {
    ...
    this.tools.set('search_components', {
      ...
      description:
        'Search TNUI (图鸟UI) components by keyword or description. Use this tool when: 1) User mentions specific component types (button, form, popup, etc.) 2) User asks for UI component recommendations 3) User needs to find components for specific use cases (navigation, forms, feedback, etc.) 4) User mentions "TNUI", "图鸟", "组件选择", "UI组件"',
      inputSchema: z.object({
        query: z
          .string()
          .describe(
            'Search keywords in Chinese or English: component names, functionality, or use cases'
          ),
        category: z
          .string()
          .optional()
          .describe(
            'Filter by component category: action-and-trigger, form-and-input, navigation-and-selection, data-display-and-status, layout-and-utility-oriented'
          ),
      }),
      handler: async (args) => {
        const results = searchComponents(args.query, args.category)
        const content = formatSearchResults(results, args.query)
    ...
    this.tools.set('generate_page', {
      name: 'generate_page',
      description:
        'Generate uniapp page code using TNUI components. Use this tool when: 1) User wants to create a new page 2) User asks "做一个页面", "创建页面", "帮我写页面" 3) User describes a page type',
      inputSchema: z.object({
        pageType: z
          .string()
          .optional()
          .describe(
            'Type of page: login, register, home, list, detail, form, user-profile, chat, settings, cart'
          ),
        pageDescription: z
          .string()
      ...
      handler: async (args) => {
        ...
        const detected = detectPageType(args.pageDescription)
        ...
        let code = generatePageCode(template)

        // Apply style preferences if provided
        if (args.stylePreferences) {
          const styles = detectStyles(args.stylePreferences)
        ...
        for (const usage of template.components) {
          ...
          if (usage.notes) response += ` (${usage.notes})`
          response += '\n'
        }
    ...
    this.tools.set('get_style_recommendation', {
      ...
      description:
        'Get TNUI component style recommendations. Use this tool when: 1) User asks about component styles or variants 2) User wants to customize component appearance 3) User mentions style keywords',
      inputSchema: z.object({
        component: z
          .string()
      ...
      handler: async (args) => {
        const styles = componentStyles.find(
          (s) => s.component === args.component
        )
        ...
        const detected = args.description ? detectStyles(args.description) : []
        const recommendations = getStyleRecommendation(
          args.component,
          args.description || ''
        )
    ...
    this.tools.set('get_component_docs', {
      ...
      description:
        'Get detailed documentation for a specific TNUI component. Use this tool when: 1) User asks for component details 2) User wants to know props, events, slots 3) User mentions specific component name',
      inputSchema: z.object({
        component: z
          .string()
          .describe(
            'Component name without tn- prefix: button, input, form, etc.'
          ),
        section: z
      ...
      handler: async (args) => {
        ...
        const doc = embeddedDocs.find(
          (d) =>
            d.uri === `tuniao://tnui/components/${componentName}` ||
            d.name === componentName
        )

        if (!doc) {
          // Try partial match
          const partialMatch = embeddedDocs.find(
            (d) =>
              d.uri.includes(`/${componentName}`) ||
    ...
    this.tools.set('list_components', {
      ...
      description:
        'List all available TNUI components. Use this tool when: 1) User asks to list all components 2) User wants to see available components 3) User asks "有哪些组件", "component list"',
      inputSchema: z.object({
        category: z
          .string()
          .optional()
          .describe(
            'Filter by category: action-and-trigger, form-and-input, navigation-and-selection, data-display-and-status, layout-and-utility-oriented'
          ),
        format: z
      ...
      handler: async (args) => {
        let components = embeddedComponents
        ...
        if (args.format === 'detailed') {
          ...
          // Group by category
          const grouped: Record<string, EmbeddedComponent[]> = {}
          for (const comp of components) {
